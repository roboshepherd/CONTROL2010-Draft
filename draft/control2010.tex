...
\title{ Scalable Multi-robot Control Using HEAD: Hybrid Event-driven Architecture with D-Bus  \thanksref{footnoteinfo}} 
...
Traditional inter-process communication (IPC) mechanisms, such as pipes, shared memory etc., fail to scale well in developing the control applications of a multi-robot system (MRS) that consists of tens of robots and various sensing and monitoring elements interconnected through several host PCs.  In this paper, we have presented a scalable, modular and flexible multi-robot control architecture that uses D-Bus as its underlying IPC method. D-Bus is a standard IPC technology for modern Linux desktop environments. It uses a message bus concepts that facilitates asynchronous data sharing among multiple processes. Here, we have described the design and practical implementation of D-Bus signal interfaces for enabling real-time interactions among  heterogeneous multi-robot control applications. Our experimental results, from a MRS implementation in a manufacturing shop-floor scenario with 16 and 32 e-puck robots, show us the feasibility of using D-Bus in scalable multi-robot control.
\end{abstract}
