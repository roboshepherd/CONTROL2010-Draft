...
\title{ Scalable Multi-robot Control Using HEAD: Hybrid Event-driven Architecture on D-Bus  \thanksref{footnoteinfo}} 
...
Traditional inter-process communication (IPC) mechanisms, such as pipes, shared memory etc., fail to scale well in developing the control applications for a relatively larger multi-robot system (MRS) that typically consists of tens of robots and various sensing and monitoring elements interconnected through several host PCs.  In this paper, we have presented a scalable, modular and flexible multi-robot control architecture, namely hybrid event-driven architecture on D-Bus (HEAD), that uses D-Bus as its underlying IPC method. D-Bus is a standard IPC technology for modern Linux desktop environments. It typically uses a message bus daemon that facilitates asynchronous data sharing among multiple processes. Here, we have shown that by using only a single type of message, namely D-Bus signal, HEAD can efficeintly enable real-time interactions among  heterogeneous multi-robot control applications. Here we have reported a MRS implementation of a manufacturing shop-floor scenario with 16 and 32 e-puck robots. The simple design of D-Bus signal interfaces of HEAD also shows doubling the number of nodes does not affect the IPC performance. 
\end{abstract}
